...
#  Builds a real Pop3Server on 127.0.0.1:2528 (self-signed cert, one
#  AUTH account), drives it with a Python smtplib client doing
#  STLS → USER/PASS → STAT → RETR, and asserts the message was delivered
...
BUILD_DIR="$(mktemp -d -t anp-smoke-XXXXXX)"
...
for i in $(seq 1 50); do
    grep -q "SMOKE-SERVER-READY" "$BUILD_DIR/srv.out" 2>/dev/null && \
      python3 -c "import socket; socket.create_connection(('127.0.0.1',2528),1).close()" 2>/dev/null && break
    sleep 0.2
done

echo "── client: STLS → USER/PASS → STAT → RETR ──"
python3 - <<'PYC'
import poplib, ssl, sys
ctx = ssl.create_default_context(); ctx.check_hostname=False; ctx.verify_mode=ssl.CERT_NONE
try:
    M = poplib.POP3("127.0.0.1", 2528, timeout=10)
    M.stls(context=ctx)
    M.user("alice"); M.pass_("s3cret")
    n, total = M.stat()
    assert n >= 1, "empty mailbox"
    resp, lines, octets = M.retr(1)
    body = b"\r\n".join(lines)
    assert b"hello pop3 smoke" in body, ("body not found: %r" % body[:120])
    M.quit()
    print("CLIENT-OK")
