...
BUILD_DIR="$(mktemp -d -t anp-tests-XXXXXX)"
...
echo "── build + run test ──"
cp "$SCRIPT_DIR/pop3session_test.am" "$PKG_DIR/_test.am"
...
check "[PASS] greeting"
check "[PASS] capa"
check "[PASS] user refused pre-tls"
check "[PASS] user/pass success"
check "[PASS] pass failure"
check "[PASS] stat"
check "[PASS] list"
check "[PASS] uidl"
check "[PASS] retr dot-stuffed"
